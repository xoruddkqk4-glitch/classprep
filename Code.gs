...
// 시트의 값(1 등)을 화면 표시용(O 등)으로 변환하는 도우미 함수
...
function getClassData(targetDateStr) {
  ...
  // 2. DailyNotes에서 해당 날짜의 세부 준비물 정보 가져오기 (1 -> O 자동 변환 적용)
  ...
  if (notesSheet && notesSheet.getLastRow() > 1) {
    ...
    // 컬럼: 날짜(0), 학급(1), 포트폴리오(2), 필기도구(3), 디벗(4), 교과서(5), 기타(6)
    for (var j = 1; j < nData.length; j++) {
      var nDateStr = "";
      if (nData[j][0] instanceof Date) {
        nDateStr = Utilities.formatDate(nData[j][0], "Asia/Seoul", "yyyy-MM-dd");
      } else {
        nDateStr = String(nData[j][0]).trim();
      }
      
      if (nDateStr === targetDateStr) {
        var nClass = String(nData[j][1]).trim();
        for (var k = 0; k < targetClasses.length; k++) {
          if (targetClasses[k].className === nClass) {
            targetClasses[k].portfolio = formatVal(nData[j][2]);
            targetClasses[k].pen = formatVal(nData[j][3]);
            targetClasses[k].dibot = formatVal(nData[j][4]);
            targetClasses[k].textbook = formatVal(nData[j][5]);
            targetClasses[k].etc = formatVal(nData[j][6]);
          }
        }
      }
    }
